...
btnSave.addEventListener("click", () => {
  ...
  if (role === "persona") {
    ...
    // === Egresado ===
      if (tipo === "egresado") {
        const anio = Number(getValue("p_grad_anio"));
        const mes = Number(getValue("p_grad_mes"));
        const dia = Number(getValue("p_grad_dia"));

        if (!Number.isInteger(anio) || anio < 1900 || anio > 2100) {
          msg.textContent = "El año de graduación no es válido.";
          msg.style.color = "#dc2626";
          return;
        }
        if (!Number.isInteger(mes) || mes < 1 || mes > 12) {
          msg.textContent = "El mes de graduación debe estar entre 1 y 12.";
          msg.style.color = "#dc2626";
          return;
        }
        if (!Number.isInteger(dia) || dia < 1 || dia > 31) {
          msg.textContent = "El día de graduación debe estar entre 1 y 31.";
          msg.style.color = "#dc2626";
          return;
        }

        profile.persona.fechaGraduacionAnio = String(anio);
        profile.persona.fechaGraduacionMes = String(mes);
        profile.persona.fechaGraduacionDia = String(dia);
...
if (role === "organizacion") {
  ...
  roleFields.innerHTML = `
    ...
    <div class="grid-2">
      <div class="field">
        ...
        <select id="p_tipoPersona" disabled>
          ...
          <option value="egresado" ${tipo==="egresado"?"selected":""}>Egresado</option>
    ...
    ${tipo === "egresado" ? renderEgresadoBlock() : ""}
...
function renderEgresadoBlock() {
  return `
    <hr class="sep">
    <h3 class="section-title">Datos del egresado</h3>

    <div class="grid-3">
      <div class="field">
        <label>Año de graduación</label>
        <input id="p_grad_anio" type="number" min="1900" max="2100"
          value="${escapeHTML(profile.persona.fechaGraduacionAnio || "")}" disabled>
      </div>

      <div class="field">
        <label>Mes</label>
        <input id="p_grad_mes" type="number" min="1" max="12"
          value="${escapeHTML(profile.persona.fechaGraduacionMes || "")}" disabled>
      </div>

      <div class="field">
        <label>Día</label>
        <input id="p_grad_dia" type="number" min="1" max="31"
          value="${escapeHTML(profile.persona.fechaGraduacionDia || "")}" disabled>
      </div>
    </div>
  `;
}
...
function syncPersonaInputsToProfile() {
  ...
  map("p_grad_anio", v => profile.persona.fechaGraduacionAnio = v.trim());
  map("p_grad_mes", v => profile.persona.fechaGraduacionMes = v.trim());
  map("p_grad_dia", v => profile.persona.fechaGraduacionDia = v.trim());
}
